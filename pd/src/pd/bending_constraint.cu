...
namespace pd
{
	BendingConstraint::BendingConstraint(SimScalar wc, VertexIndexType center_vertex, const std::vector<VertexIndexType>& neighbor_vertices, const PositionData& positions, bool discard_quadratic_term) : Constraint(wc, 1 + neighbor_vertices.size()), discard_quadratic_term(discard_quadratic_term)
	{
	...
	__host__ void BendingConstraint::precompute_laplacian_weights(const std::vector<VertexIndexType>& neighbor_vertices, const PositionData& positions)
	{
		...
		laplacian_weights[0] = 0; // init value
