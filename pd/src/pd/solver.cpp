...
namespace pd {
	...
	void Solver::free_cuda_memory()
	{
    	gpu_local_solver.reset();
	}

	void Solver::precompute_A(const std::unordered_map<MeshIDType, DeformableMesh>& models)
	{
		...
		for (const auto& [id, model] : models)
		{
			...
			for (const auto& constraint : model.get_all_constraints())
			{
			...
			for (int i = 0; i < n; i++)
			{
				for (int j = 0; j < 3; j++)
				{
					A_triplets.emplace_back(3 * acc + 3 * i + j, 3 * acc + 3 * i + j, model.masses()(i) * dtsqr_inv);
				}
			}
			acc += n;
		}
	...
	void Solver::precompute(const std::unordered_map<MeshIDType, DeformableMesh>& models)
	{
		timer.start();
	...
	void Solver::set_solver(pd::LinearSysSolver sel)
	{
	...
	)
	{
		...
		for (const auto& [id, model] : models)
		{
			...
			const VelocityData& v = model.velocities();

			// auto fails
			const DataMatrixX3 a = f_exts.at(id).array().colwise() / (model.masses()).array(); // M^{-1} f_{ext}
		...
		for (const auto& [id, model] : models)
		{
			...
			for (int i = 0; i < n; i++)
			{
				...
				m_i.setZero();
				for (int j = 0; j < 3; j++)
					m_i(j, j) = static_cast<float>(model.masses()(i));
		...
		for (int k = 0; k < n_itr; k++)
		{
			...
			use_gpu_for_local_step ? local_step_gpu(q_nplus1, b) : local_step_cpu(models, q_nplus1, b);
	...
	void Solver::local_step_cpu(const std::unordered_map<MeshIDType, DeformableMesh>& models, const SimPositions& q_nplus1, SimVectorX& b)
	{
		int acc = 0;
		for (const auto& [id, model] : models)
		{
	...
	void Solver::set_dirty(bool val)
	{
		dirty = val;
	...
	void Solver::set_linear_sys_solver(int idx)
	{
		linear_sys_solver = solvers.begin() + idx;
	}

	Float Solver::get_precompute_progress() { return progress_percentage.get_progress(); }
}
