...
namespace ui {
	...
	template void mesh_manager_menu<pd::DeformableMesh>(const std::string& obj_prompts, const std::unordered_map<int, pd::DeformableMesh>& meshes, ObjManager& obj_manager, pd::UserControl& user_control, bool is_animating);
	template void mesh_manager_menu<std::unique_ptr<primitive::Primitive>>(const std::string& obj_prompts, const std::unordered_map<int, std::unique_ptr<primitive::Primitive>>& meshes, ObjManager& obj_manager, pd::UserControl& user_control, bool is_animating);
	...
	)
    {
		if (ImGui::CollapsingHeader("Visualization Setting"), ImGuiTreeNodeFlags_DefaultOpen)
		{
			...
			ImGui::Checkbox("Headless Mode", &user_control.headless_mode);
	...
	void instantiator_menu(instancing::Instantiator& instantiator)
	{
		...
		// auto invoke only once for test
    ...
    {
		pd::Solver& solver = pd::Solver::instance();
        if (ImGui::CollapsingHeader("Simulation Control"), ImGuiTreeNodeFlags_DefaultOpen)
		{
			...
			if (ImGui::BeginCombo(LABEL("current solver"), items[cur_select_item_idx]))
			{
				for (int i = 0; i < IM_ARRAYSIZE(items); i++)
				{
					...
					{
						// If solver changed
						if (solver_params.selected_solver != static_cast<pd::LinearSysSolver>(i))
						{
						...
						solver_params.selected_solver = static_cast<pd::LinearSysSolver>(i);
					}
					if (is_selected)
					{
						ImGui::SetItemDefaultFocus();
